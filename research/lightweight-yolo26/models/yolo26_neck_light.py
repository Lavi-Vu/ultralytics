"""YOLO26-N with Lightweight Neck Topology (Track 2, H5).

FPN only (PAN removed). Output channels to Detect:
  P3 = _w(256) = 64, P4 = _w(512) = 128 (from n11), P5 = _w(1024) = 256 (from b10)
"""
...
class YOLO26NeckLight(nn.Module):
    """YOLO26-Nano with FPN-only neck (PAN removed)."""
    ...
    def __init__(self, nc=80, reg_max=1, end2end=True):
        ...
        # Neck — FPN only (2 GhostC3k2 blocks), no PAN
        self.n_up1 = nn.Upsample(scale_factor=2, mode="nearest")
        self.n_cat1 = _Concat()
        self.n11 = GhostC3k2(_w(1024) + _w(512), _w(512), _d(2), True)  # 384→128
        self.n_up2 = nn.Upsample(scale_factor=2, mode="nearest")
        self.n_cat2 = _Concat()
        self.n12 = GhostC3k2(_w(512) + _w(512), _w(256), _d(2), True)   # 256→64
    ...
    def forward(self, x):
        ...
        # No PAN: P4 = n11, P5 = b10
        return self.detect([p3, n11, b10])
