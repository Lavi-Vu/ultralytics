...
def channel_shuffle(x, groups=2):
    ...
    idx = torch.arange(C, device=x.device).reshape(groups, step).transpose(0,1).reshape(-1)
    # Apply permutation directly on channel dimension
    return x[:, idx, :, :]
...
class GSConv(nn.Module):
    ...
    def forward(self, x):
        ...
        return channel_shuffle(x2, groups=2)
    # def forward(self, x):
    #     x1 = self.cv1(x)
    #     x2 = torch.cat((x1, self.cv2(x1)), 1)
        # shuffle
